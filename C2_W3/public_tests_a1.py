...
def test_eval_mse(target):
    ...
    y_hat = np.array([3.0] * 10)
    y_tmp = np.array([3.0] * 10)
    result = target(y_hat, y_tmp)
    assert np.isclose(result, 0.0), f"Wrong value. Expected 0.0 when y_hat == t_tmp, but got {result}"
    ...
    y_hat = np.array([3.0] * 5)
    y_tmp = np.array([2.0] * 5)
    result = target(y_hat, y_tmp)
    assert np.isclose(
        result, 0.5
    ), f"Wrong value. Expected 0.5, but got {result}. Remember to divide by (2*m)"
...
def test_eval_cat_err(target):
    ...
    result = target(y_hat, y_tmp)
    assert not np.isclose(result, 6.0), f"Wrong value. Expected 1, but got {result}. Did you divided by m?"

    y_hat = np.array([1, 2, 0])
    y_tmp = np.array([1, 2, 3])
    result = target(y_hat, y_tmp)
    assert np.isclose(result, 1.0 / 3.0, atol=1e-6), f"Wrong value. Expected 0.333, but got {result}"

    y_hat = np.array([1, 0, 1, 1, 1, 0])
    y_tmp = np.array([1, 1, 1, 0, 0, 0])
    result = target(y_hat, y_tmp)
    assert np.isclose(result, 3.0 / 6.0, atol=1e-6), f"Wrong value. Expected 0.5, but got {result}"
...
def model_test(target, classes, input_size):
    ...
    for layer in target.layers:
        ...
        assert layer.output.shape == tuple(
            expected[i][1]
        ...
        assert layer.kernel_regularizer is None, "You must not specify any regularizer for any layer"
        i += 1

    assert (
        type(target.loss) == SparseCategoricalCrossentropy
    ), f"Wrong loss function. Expected {SparseCategoricalCrossentropy}, but got {target.loss}"
    ...
    assert np.isclose(
        lr, expected_lr, atol=1e-8
    ), f"Wrong learning rate. Expected {expected_lr}, but got {lr}"
...
def model_r_test(target, classes, input_size):
    ...
    for layer in target.layers:
        ...
        if not (expected[i][3] == None):
            assert (
                type(layer.kernel_regularizer) == expected[i][3][0]
            ), f"Wrong regularizer. Expected L2 regularizer but got {type(layer.kernel_regularizer)}"
            assert np.isclose(
                layer.kernel_regularizer.l2, expected[i][3][1]
            ), f"Wrong regularization factor. Expected {expected[i][3][1]}, but got {layer.kernel_regularizer.l2}"
        else:
