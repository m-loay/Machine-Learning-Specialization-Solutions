...
def test_c1(target):
    ...
    for i, (layer_type, output_shape, activation) in enumerate(expected_layers):
        layer = target.layers[i]
        assert isinstance(layer, layer_type), \
            f"Layer {i} is wrong type. Expected {layer_type} but got {type(layer)}"
        assert layer.output.shape == output_shape, \
            f"Layer {i} has wrong shape. Expected {output_shape} but got {layer.output_shape}"
        assert layer.activation.__name__ == activation, \
            f"Layer {i} has wrong activation. Expected {activation} but got {layer.activation.__name__}"
