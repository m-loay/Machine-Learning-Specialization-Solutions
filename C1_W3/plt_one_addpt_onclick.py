...
class plt_one_addpt_onclick:
    ...
    def __init__(self, x, y, w, b, logistic=True):
        ...
        ax.scatter(
            x[neg], y[neg], marker="o", s=100, label="benign", facecolors="none", edgecolors=dlblue, lw=3
        )
    ...
    def calc_linear(self, event):
        ...
        for it in [1, 1, 1, 1, 1, 2, 4, 8, 16, 32, 64, 128, 256]:
            ...
            self.aline = self.ax[0].plot(
                self.x, y_hat, color=dlblue, label=f"y = {np.squeeze(self.w):0.2f}x+({self.b:0.2f})"
            )
    ...
    def calc_logistic(self, event):
        ...
        for it in [1, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
            ...
            self.aline = self.ax[0].plot(x_hat, y_hat, color=dlblue, label="y = sigmoid(z)")
            f_wb = np.matmul(x_hat.reshape(-1, 1), self.w) + self.b
            self.bline = self.ax[0].plot(
                x_hat, f_wb, color=dlorange, lw=1, label=f"z = {np.squeeze(self.w):0.2f}x+({self.b:0.2f})"
            )
            self.alegend = self.ax[0].legend(loc="lower right")
            time.sleep(0.3)
            self.fig.canvas.draw()
    ...
    def draw_thresh(self):
        ...
        c = self.ax[0].annotate(
            "Malignant", xy=[xp5, 0.5], xycoords="data", xytext=[30, 5], textcoords="offset points"
        )
        d = FancyArrowPatch(
            posA=(xp5, 0.5),
            posB=(xp5 + 1.5, 0.5),
        ...
        e = self.ax[0].annotate(
            "Benign", xy=[xp5, 0.5], xycoords="data", xytext=[-70, 5], textcoords="offset points", ha="left"
        )
        f = FancyArrowPatch(
            posA=(xp5, 0.5),
            posB=(xp5 - 1.5, 0.5),
    ...
    def resize_sq(self, bcid):
        ...
        try:
            # Try to access the rectangles
            if hasattr(bcid, "_rectangles"):
                rectangles = bcid._rectangles
            elif hasattr(bcid, "rectangles"):
                rectangles = bcid.rectangles
            else:
                print("Unable to find rectangles attribute")
                return
            ...
            if len(rectangles) > 0:
                ...
                rect.set_height(3 * height)

                # Update the position of the checkmark
                ymax = rect.get_bbox().y1
                ymin = rect.get_bbox().y0

                # Assuming the checkmark is made of two lines
                if hasattr(bcid, "lines") and len(bcid.lines) > 0 and len(bcid.lines[0]) >= 2:
                    bcid.lines[0][0].set_ydata([ymax, ymin])
                    bcid.lines[0][1].set_ydata([ymin, ymax])

            # Redraw the canvas to show the changes
            bcid.ax.figure.canvas.draw()

        except Exception as e:
            print(f"An error occurred while resizing the checkbox: {e}")
            # Optionally, you can print the full traceback for debugging
            # import traceback
        # traceback.print_exc()
