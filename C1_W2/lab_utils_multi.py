...
def plt_contour_wgrad(
    ...
    CS = ax.contour(
        w0, b0, z, contours, linewidths=2, colors=[dlblue, dlorange, dldarkred, dlmagenta, dlpurple]
    )
...
def plt_contour_multi(x, y, w, b, ax, prange, p1, p2, title="", xlabel="", ylabel=""):
    ...
    CS = ax.contour(
        px, py, z, contours, linewidths=2, colors=[dlblue, dlorange, dldarkred, dlmagenta, dlpurple]
    )
...
def load_house_data():
    data = np.loadtxt("../_resources_ML_spec/C1_W2/data/houses.txt", delimiter=",", skiprows=1)
    X = data[:, :4]
    y = data[:, 4]
    return X, y
